...
# mod_FILE = './mod/note-合集0508.md'
mod_FILE = './mod/note-合集.md'

# K_test = 200
K_test = None
input_file = './data/待打标.csv'
...
def get_content_by_type(row, type):
    # try:
    with open(f'./mod/note-{type}.md', 'r', encoding='utf-8') as file:
        note_content = file.read()
    # except FileNotFoundError:
    #     logger.error(f"note-{type}.md 文件未找到")
    #     return {}
...
def save_results(row_results, input_file):
    ...
    valid_collections = [
        ...
        '自尊心','我很好', '积极心态', '愉悦心情', '焕发生命力', '点燃动力', '自信', '拥抱女性身份', '女性力量', '亲密关系',
...
def process_row(index, row, checkpoint_file, input_file, pbar):
    ...
    for type in note_types:
        ...
        # print(f"获取{type}标注成功: {row_results}")
...
def main():
    ...
    if choice == "1":
        process_file(input_file, k=K_test)
